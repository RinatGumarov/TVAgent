 ...
 *
 * Right-side sidebar per the plan's §14: prompt box, execution trace with
 * collapsible tool calls, confirmation dialogs, and a settings drawer.
 */
(() => {
  'use strict';
  ...
  function wireSettings() {
    ...
    function migrate(s) {
      ...
      // Provider was openai, so the shared slots held its key and model — with
      // one exception: an Anthropic key in there came from the Anthropic field
      // and belongs on that side, not as a bearer token for someone else.
    ...
    /**
     * Fills the model dropdown from the provider itself. The list belongs to a
     * base URL, so it is fetched once per URL and again when that URL changes.
     * The field stays a plain text input: if nothing comes back, the user types
     * the name as before.
     */
    let listedFor = null;
    loadModels = async () => {
    ...
    // Switching provider is only a switch: each side keeps its own key and
    // model, so nothing is copied across and nothing is overwritten. The worker
    // reads the list from storage, so the write has to land before the ask.
