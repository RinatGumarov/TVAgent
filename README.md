...
_Adding EMA 50 and 200, with reasoning on: the model's thinking is shown
unfolded above its tool calls._

![Marking the high and low of the visible range](.github/media/levels.avif)

_Marking the high and low of the visible range, with reasoning off: the model
goes straight to the tool calls._

![Writing an EMA crossover strategy in Pine and backtesting it](.github/media/strategy.avif)

_Building an EMA crossover strategy: the agent writes the Pine script in
TradingView's Pine Editor, adds it to the chart and reads the backtest report._
