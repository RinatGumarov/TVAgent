...

{
  ...
  check(
    'long enough not to be searched through',
    [...ids].every((id) => id.length >= 32),
    true,
  );
}
...

{
  ...
  check(
    'a stamp is deterministic for one key, id and role',
    await wire.stamp(key, 'id-1', 'req'),
    req,
  );
  ...
  check(
    'the response stamp differs from the request stamp',
    (await wire.stamp(key, 'id-1', 'res')) === req,
    false,
  );
  check('another id gives another stamp', (await wire.stamp(key, 'id-2', 'req')) === req, false);
  check(
    'and another key gives another stamp',
    (await wire.stamp(other, 'id-1', 'req')) === req,
    false,
  );
