...

{
  ...
  check(
    'the permission does not widen to every HTTP host',
    loopback.permission,
    'http://127.0.0.1/*',
  );
}

{
  ...
  check(
    'a hosted HTTPS endpoint is allowed and normalized',
    hosted.baseUrl,
...
check(
  ...
  /HTTPS|localhost|127\.0\.0\.1/.test(rejected('http://api.example.com/v1') || ''),
  true,
);
check(
  'credentials embedded in a URL are refused',
  /credentials/i.test(rejected('https://key:secret@api.example.com/v1') || ''),
  true,
);
check(
  'query parameters are refused',
  /query|fragment/i.test(rejected('https://api.example.com/v1?key=secret') || ''),
  true,
);
check('non-HTTP protocols are refused', /HTTP/i.test(rejected('file:///tmp/model') || ''), true);
check(
  'an invalid URL is refused in the user’s terms',
  /valid URL/i.test(rejected('not a url') || ''),
  true,
);
