/**
 * TVAgent — the panel shell: header, context row, empty state, composer, boot.
 *
 * Runs the real panel.js under the shared fake DOM and fake
 * TVAgentMount/TVAgentSettings/TVAgentBridge/TVAgentRuntime/TVAgentChat. This
 * file is the only one that drives those five together, so it fakes their
 * interfaces rather than their insides — the insides already have their own
 * tests in mount-test.mjs, settings-screen-test.mjs and chat-test.mjs.
 *
 * The DOM comes from helpers/dom.mjs. panel.js is the module that needs the
 * most of it: insertAdjacentHTML (build() appends rather than assigns, so the
 * overlay resizer already inside root survives), `disabled` as both a markup
 * attribute and a property, `value` on the textarea, `style`, `scrollHeight`,
 * focus() with a call count, and setAttribute — panel.js is the only caller of
...
/**
 * A ResizeObserver stub. panel.js reaches for it as window.ResizeObserver
 * rather than as a bare global for exactly this reason: node has none. It
 * keeps the callback and the observed nodes, and resize(width) calls back with
 * the same entries contract the real one uses — panel.js reads the last entry.
 */
 ...
 * the reports the driver pushes afterwards. `pushChart` is how a test plays
 * the part of a symbol change.
 */
 ...
 *
 * This matters for the boot-ordering tests. panel.js awaits an async function,
 * so the promise it waits on is not the one the mock handed back but one that
 * FOLLOWS it, a microtask later. Awaiting the mock's own promise in the test
 * therefore resolves a tick EARLIER than what boot() is really waiting for,
 * and a check like "build() has run by now" catches it not-yet-run — not
 * because the order is wrong, but because the test looked too early. Any async
 * wrapper around a test-controlled promise adds the same extra tick;
 * setTimeout(0) waits out however many there are.
 */
function flush() {
...
function load({ mount, settings, bridge, runtime, chat }) {
  ...
  // The shared model catalog, which is where the composer chip's label comes
  // from. Loaded the way the manifest loads it, ahead of panel.js.
...
/**
 * A complete, successful boot with default mocks — any part can be overridden.
 * Returns the nodes and the agent, once boot() has really run to the end.
 */
...

{
  ...
  // Not `await mountPromise` here: mount.mount() above is itself async, so
  // the promise it returns follows mountPromise by an extra microtask — see
  // the comment on flush(), which waits that out reliably.
  await flush();
...
// boot() used to be a bare call with no .catch, so a failed mount(), a throw
// from TVAgentSettings.create(), a rejected settings.ready or a rejected
// probeWhenReady() left the header, context row and composer built, the status
// frozen at "connecting…" and no error anywhere on screen. One block per
// failure point below — with mount() getting its own, because when it fails
// there is no panel to write an error into.

{
  // (1) mount() fails. build() has not run, so there is no root, no chat and
  // no statusEl — the ordinary showError() has nothing to build from. This is
  // the other path: a standalone banner appended straight to
...
{
  // (2) TVAgentSettings.create() throws synchronously. build() has already
  // run — root, chat and statusEl exist — so there is somewhere to write the
  // error the ordinary way: showError() → setStatus/setEmpty/chat.error.
...
{
  // (3) settings.create() returns, but its `ready` is a rejected promise.
  const settings = {
    create() {
      return {
...
{
  // (4) probeWhenReady() rejects. bridge.js does not do that today — it
  // catches call() errors and returns a report — but boot() must not rely on
  // that and has to survive a rejection if one ever happens.
...
// The formatting formula is spelled out here exactly as panel.js spells it —
// Intl.NumberFormat(undefined, {min:2, max: <2|6>}) — so the test does not
// depend on the current locale's grouping character (a comma in en-US, a space
// in many European locales) and still catches the real regressions: the wrong
// field read, the join with symbol/resolution lost, or the "under $1"
// threshold moved or inverted.
...
{
  // An instrument under $1: two decimals would round it to 0.00. The
  // "abs(price) < 1" threshold has to widen it to six.
...
{
  // The chip names what actually travels to the model in the system prompt:
  // the symbol and the timeframe. The price does not — the agent reads that
  // with its own tools — so "… 64,446.70 in context" was saying something
  // untrue. The price stays in the row above, in the popover and in the
  // chip's title.
  const h = await bootedPanel({ caps: caps({ symbol: 'BTCUSDT', resolution: '240', price: 121480 }) });
...
{
  // "240" is four hours, but only if you already know that. The panel writes
  // what TradingView's own interval button writes. Anything unrecognised is
  // passed through untouched rather than guessed at — range bars and ticks
  // arrive here exactly that way.
  const cases = [
    ['1', '1m'],
    ['45', '45m'],
    ['60', '1h'],
...

{
  ...
  // Enter always calls submit() — the keydown handler does not check busy
  // itself, submit() does — so this is the way to exercise submit()'s own busy
  // guard rather than the button's branch, which during a run cancels instead
  // of sending (see below).
...

{
  ...
  // endRun has to recompute `disabled` from the CURRENT input value rather
  // than inherit false from startRun or always set true. Typing during the run
  // is a thing people really do, so the input is changed mid-run and endRun
  // has to see it.
...

{
  ...
  h.agent.handlers.onDone({ stopReason: 'end_turn' });
  check('endRun with an empty input at that moment disables it again', h.sendBtn.disabled, true);
}
...
{
  // The message goes into the list directly, bypassing submit() and its own
  // particular call order (that has its own block below), and then settings is
  // opened and closed again: toggleSettings() always calls showScreen(), which
  // always recomputes isEmpty() from the CURRENT list — the one path that
  // reliably re-evaluates the empty state.
  const h = await bootedPanel();
  const dummy = h.doc.createElement('div');
  dummy.className = 'tva-msg user';
  h.listEl.appendChild(dummy);

  click(h.root.querySelector('#tva-gear')); // settings: empty must be hidden
  check('in settings the empty state is hidden whatever the list holds', h.emptyEl.classList.contains('tva-hidden'), true);

  click(h.root.querySelector('#tva-gear')); // back to chat
  check('back in chat with a non-empty list, the empty state is hidden', h.emptyEl.classList.contains('tva-hidden'), true);
}

{
  // The same round trip, but the list stays empty — so must the empty state.
  const h = await bootedPanel();
  click(h.root.querySelector('#tva-gear'));
  click(h.root.querySelector('#tva-gear'));
  check('back in chat with an empty list, the empty state returns', h.emptyEl.classList.contains('tva-hidden'), false);
}

section('screens: submit() — the first message hides the empty state at once');
{
  // submit() used to call setEmpty(false), then showScreen('chat') — which
  // recomputes the empty state from the CURRENT list length — and only THEN
  // chat.user(trimmed) to add the message. So showScreen saw an empty list and
  // put the empty state back, over the very first message. chat.user(trimmed)
  // now runs BEFORE showScreen('chat'), so the recompute sees a non-empty
  // list. That is the invariant that was wanted all along: the empty state
  // shows only while the list is empty and we are not in settings, with no
  // exception carved out for the first message.
  const h = await bootedPanel();
  ...
  // The second message: the list was already non-empty before this call, so
  // the chat.user()/showScreen() order could not mask a regression here. The
  // check on the first message above is the only one that catches it.
...
// ============================================================================
section('composer: the chips sit under the field, the send button inside it');

{
  ...
  // The model and context chips used to live inside the field's border and
  // share a row with the send button: a long model name wrapped inside its own
  // chip and pushed the button past the right edge. They have their own row
  // under the field now.
  check('the model chip is outside the field', field.contains(h.root.querySelector('#tva-model-chip')), false);
  check('the context chip is outside the field', field.contains(h.ctxChipEl), false);
  check('both chips sit in their own row', chipRow.contains(h.ctxChipEl) && chipRow.contains(h.root.querySelector('#tva-model-chip')), true);
  check('the send button stayed in the field', field.contains(h.sendBtn), true);

  // The popover opens upward from the chip row rather than from the whole
  // composer; otherwise it would float over the field, detached from the chip
  // that opened it.
  check('the popover lives in the chip row, which anchors it', chipRow.contains(h.ctxPopEl), true);
}
...

{
  ...
  // A hidden panel — display:none on the overlay, an inactive widget bar page
  // — measures 0. That is "not rendered", not "narrow": otherwise every close
  // would silently switch the layout underneath it.
  h.resize(0);
...

{
  ...
  // CSS hides the word "connected" when narrow, not JS: the text stays in the
  // DOM (and in the title), or a screen reader would lose the status along
  // with the layout.
...

{
  ...
  // A click inside the popover itself — on the symbol row — must not close
  // it: people select the text in there with the mouse.
...

{
  ...
  check('another key does not close it', h.ctxPopEl.classList.contains('tva-hidden'), false);

  h.doc.fire('keydown', { key: 'Escape' });
  check('Escape closes it', h.ctxPopEl.classList.contains('tva-hidden'), true);
}

{
  // Widening puts the whole line back on the chip, and a popover over it
  // would repeat an answer that is already on screen.
  const h = await bootedPanel();
  h.resize(260);
  click(h.ctxChipEl);
  h.resize(400);
  check('widening closes an open popover', h.ctxPopEl.classList.contains('tva-hidden'), true);
  check('and the listeners come off here too', h.doc.listenerCount('click'), 0);
}

{
  // The settings screen hides the whole composer, so the popover would be
  // floating over nothing.
  const h = await bootedPanel();
  h.resize(260);
  click(h.ctxChipEl);
  click(h.root.querySelector('#tva-gear'));
  check('opening settings closes the popover', h.ctxPopEl.classList.contains('tva-hidden'), true);
}

section('the chart changes under the panel');

// The capability report is what the system prompt names and what the tool list
// is filtered by, and it used to be read once, at boot. After a symbol change
// the prompt still announced the old ticker, and a symbol whose bars had not
// loaded when the panel started kept get_series_data switched off for the whole
// session.

{
  ...
  // In place, not replaced: the agent was handed this exact object at boot and
  // reads it on every turn, so a fresh one would leave it on the old report
  // while looking, from here, as though it had worked.
...
{
  // `price` is absent from the report, not null, whenever the new symbol's bars
  // have not loaded — probe() leaves it off rather than reading a half-loaded
  // series. Merged onto the old report it simply was not overwritten, and the
  // context row showed the new ticker at the old symbol's price.
  const h = await bootedPanel({
...
{
  const h = await bootedPanel();
  // The label comes from the shared catalog rather than a map written out
  // here — it used to be spelled out in four places at once.
