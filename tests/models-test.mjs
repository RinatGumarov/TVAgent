/**
 * TVAgent — the model list behind the settings dropdown.
 *
 * Runs the real listModels() from service-worker.js against a stubbed fetch:
 * parsing /v1/models, the tool-support marks Ollama's /api/show provides, the
 * ordering, and what happens when there is no server. It finishes against a
 * live Ollama if one is running.
 *
...
section('a live Ollama');
{
  ...
  if (!up) {
    console.log(' skip  ollama is not running');
  } else {
    const { listModels } = load(fetch);
    const models = await listModels(OLLAMA);
    check('models came back', models.length > 0, true);
    check('every one is annotated', models.every((m) => typeof m.tools === 'boolean'), true);
