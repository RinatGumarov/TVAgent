/**
 * TVAgent — the agent runtime.
 *
 * Runs the real agent.js with the worker port and the page bridge faked, so
 * the model turn and the tool call are both under the test's control. What is
 * expensive to break here is the lifecycle: a run can end at any await, and
 * every one of these cases used to leave the conversation in a state the API
 * rejects for good — recoverable only by starting a new chat.
 *
 *   - New chat while a run is in flight: the old loop kept writing, and its
 *     first write landed at the head of an empty history as an assistant turn;
 *   - Stop landing on the last tool of a batch: the batch was answered twice,
 *     which is two tool_result blocks for one tool_use_id;
 *   - cancel() then send(): the abandoned loop came back to life and two loops
 *     wrote into one conversation;
 *   - Allow pressed after Stop: the level-2 tool ran anyway, overwriting the
 *     user's Pine minutes after they had ended the run.
 *
 * And the permission model: a name that is not a tool is refused rather than
 * offered to the user as something to allow, which is what let inherited
 * properties and internal driver methods through.
 *
 *   node agent-test.mjs
 */
...
/**
 * A worker port the test drives by hand. `answer` hands back one assistant
 * turn; until it is called the agent is parked in #turn(), which is where a
 * Stop most often lands.
 */
function makePort() {
  const port = {
    posted: [],
...
function setup({ capabilities = { pine: true, series: true, strategy: true }, tool } = {}) {
  ...
  // The real tool list, so the permission levels under test are the shipped
  // ones rather than a convenient copy.
  ...
  // Confirmation is a promise the test resolves when it chooses, because the
  // whole question is what happens while the card is on screen.
...

{
  ...
  // a property every object inherits. All three used to be treated as level 3
  // and put to the user as something to allow.
...
{
  // Auto-approve is labelled "Run Pine edits without asking". It covers
  // level 2 and stops there.
  const h = setup();
...
/**
 * A turn that came back with nothing in it.
 *
 * The worker drops content blocks the API will not accept back — an empty text
 * block is the usual one, left behind by a turn that opens with text and goes
 * straight to a tool call. When that is the only block there was, what arrives
 * here is an empty array, and an assistant message with empty content is
 * rejected by the same API for the same reason. It would then be sent again
 * with every later message, so the conversation is finished: the run has to end
